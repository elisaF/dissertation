...
%or by explicitly allowing annotators to select more than one label. 
...
%However, there is less research examining the cause of ``genuine'' ambiguities-- whether they are a result of separate \emph{subjective} interpretations or of multiple \emph{simultaneous} interpretations.  
...
%\footnote{PDTB 3.0 also allows multiple labels for discourse relations.} 
...
%\citet{Versley:2011} points to the value of having ambiguous connectives that are helpful in identifying when more than one discourse relation can hold. 
...
Because questions heavily influence the type of answer, I also plan to annotate the type of question to understand whether they are ``loaded'', for example by containing presuppositions or implicatures. 
...
\begin{minipage}{.5\textwidth}
  ...
  \captionof{figure}{\textsc{Multi-label}: predict the set of dialogue acts for a question-answer pair (qa), while modeling correlations within a set (blue).}
...
\begin{minipage}{.5\textwidth}
  ...
  \captionof{figure}{\textsc{Multi-label+context}: predict the set of dialogue acts for a question-answer pair and history of question-answer pairs, while modeling correlations within a set (blue) and across time steps (red).}
...
\subsection{Crowdsourcing Phase 2.}
\label{subsec:phase2}
